    ...
    for channelpair = 1:size(channelcombi,1)
        ...
        for t4t = 1:nrtrials(channelpair,2)
            ...
            if strcmp(cfg.shuffle, 'no')
                ...
                trial2 = trial1;
                timespan2 = timeindices(1):timeindices(2);
                timespan1 = timespan2;
            ...
            elseif strcmp(cfg.shuffle, 'yes')
                ...
                switch cfg.surrogatetype
                    ...
                    case 'blockresampling'
                        ...
                        timespan2 = [cutpoint:timeindices(2),timeindices(1):cutpoint-1];
                        timespan1 = timeindices(1):timeindices(2);
                    ...
                    case 'trialreverse'
                        ...
                        timespan1 = timeindices(1):timeindices(2);
                        timespan2 = timeindices(1):timeindices(2);
                        timespan2 = flipdim(timespan2,2);
                        
                    case 'blockreverse1'
                        trial2 = t4t;
                        cutpoint = round( (timeindices(2)-timeindices(1)+1) * rand(1));
                        timespan2 = flipdim([cutpoint:timeindices(2),timeindices(1):cutpoint-1],2);
                        timespan1 = timeindices(1):timeindices(2);

                    case 'blockreverse2'
                        trial2 = t4t;
                        cutpoint = round( (timeindices(2)-timeindices(1)+1) * rand(1));
                        timespan2 =[flipdim(cutpoint:timeindices(2),2),timeindices(1):cutpoint-1];
                        timespan1 = timeindices(1):timeindices(2);
                        
                    case 'blockreverse3'
                        trial2 = t4t;
                        cutpoint = round( (timeindices(2)-timeindices(1)+1) * rand(1));
                        timespan2 =[cutpoint:timeindices(2),flipdim(timeindices(1):cutpoint-1,2)];
                        timespan1 = timeindices(1):timeindices(2);
                    
                    case 'blockreverse4'
                        trial2 = t4t;
                        
                        len=timeindices(2)-timeindices(1)+1;
                        xr=round(abs(randn(len,1)*1000)); %xr=geornd(.001*ones(len,1));
                        xr=cumsum(xr);
                        xr=xr(logical((xr<(len-1)).*(xr>1)));
                        xr=[[1;xr+1] [xr;len]];
                        xr=xr(randperm(size(xr,1)),:);
                        
                        t=[];
                        for c=1:size(xr,1)
                            t=vertcat(t,(xr(c,1):xr(c,2))');
                        end
                        t=flipud(t);
                        timespan2 = t'+timeindices(1)-1;
                        timespan1 = timeindices(1):timeindices(2);
            ...
            if shifttest == 1
                ...
                timespan2(timespan2==0)=[];
                a=squeeze(data4TE{channelpair,1}(trial1,timespan2));
                b=squeeze(data4TE{channelpair,2}(trial2,timespan1));
                if isfield(data, 'Data4Embedding') && strcmp(data.TEprepare.cfg.datatype, 'fMRI')
                    a_e=squeeze(embcell{channelpair,1}(trial1,:,timespan2));
                    b_e=squeeze(embcell{channelpair,2}(trial2,:,timespan1));
                end
            end
            t = tic;
    ...
    for channelpair = 1:size(channelcombi,1)
        ...
        parfor t4t = 1:nrtrials(channelpair,2)
            ...
            elseif strcmp(cfg.shuffle, 'yes')
                ...
                elseif strcmp(cfg.surrogatetype,'blockreverse4')
                    trial2 = t4t;
