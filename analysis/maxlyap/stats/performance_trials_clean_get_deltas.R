...
filename_scores_in <- 'Scores_MaxLyap_2024-06-07.csv'
...
xd <- x[x$training_phase_label=='PreTest',] %>%
  ...
  summarise(pre_score = mean(score),
            ...
            pre_max_lyap_stim_short = mean(max_lyap_stim_short),
            pre_max_lyap_stim_long = mean(max_lyap_stim_long),
            pre_max_lyap_user_short = mean(max_lyap_user_short),
            pre_max_lyap_user_long = mean(max_lyap_user_long)) %>%
  ungroup()
...
x$max_lyap_stim_short_delta <- 100*(x$max_lyap_stim_short - x$pre_max_lyap_stim_short)/x$pre_max_lyap_stim_short
x$max_lyap_stim_long_delta <- 100*(x$max_lyap_stim_long - x$pre_max_lyap_stim_long)/x$pre_max_lyap_stim_long
x$max_lyap_user_short_delta <- 100*(x$max_lyap_user_short - x$pre_max_lyap_user_short)/x$pre_max_lyap_user_short
x$max_lyap_user_long_delta <- 100*(x$max_lyap_user_long - x$pre_max_lyap_user_long)/x$pre_max_lyap_user_long
...
x_test_ave <- x_test %>%
  ...
  summarise(score = mean(score),
            ...
            max_lyap_stim_short = mean(max_lyap_stim_short),
            max_lyap_stim_long = mean(max_lyap_stim_long),
            max_lyap_user_short = mean(max_lyap_user_short),
            max_lyap_user_long = mean(max_lyap_user_long)) %>%
  ungroup() %>%
...
x_test_ave_wide <- x_test_ave %>%
  pivot_wider(names_from = training_phase_label, 
              values_from = c('score','c','pitch_error',
                              'max_lyap_stim_short','max_lyap_stim_long',
                              'max_lyap_user_short','max_lyap_user_long'))
...
diff_data <- fun_ave_pivot_diff(x_test, x_test$max_lyap_stim_short)
x_delta <- x_delta %>% left_join(., diff_data, by = c("pp","task_label","Training","TrainingPhase"))

diff_data <- fun_ave_pivot_diff(x_test, x_test$max_lyap_stim_long)
x_delta <- x_delta %>% left_join(., diff_data, by = c("pp","task_label","Training","TrainingPhase"))

diff_data <- fun_ave_pivot_diff(x_test, x_test$max_lyap_user_short)
x_delta <- x_delta %>% left_join(., diff_data, by = c("pp","task_label","Training","TrainingPhase"))

diff_data <- fun_ave_pivot_diff(x_test, x_test$max_lyap_user_long)
x_delta <- x_delta %>% left_join(., diff_data, by = c("pp","task_label","Training","TrainingPhase"))
rm(diff_data)
...
names(x_delta) <- c("pp","task_label",
                  ...
                  "mlss_PreTest","mlss_PostTest","mlss_Retention","mlss_Delta",
                  "mlsl_PreTest","mlsl_PostTest","mlsl_Retention","mlsl_Delta",
                  "mlus_PreTest","mlus_PostTest","mlus_Retention","mlus_Delta",
                  "mlul_PreTest","mlul_PostTest","mlul_Retention","mlul_Delta")
...
x_delta_wider <- x_delta %>%
  select(pp, task_label, Training, TrainingPhase, c_Delta, pitch_error_Delta, score_Delta,
         mlss_Delta, mlsl_Delta, mlus_Delta, mlul_Delta) %>%
  pivot_wider(names_from = TrainingPhase, 
              values_from = c('c_Delta','pitch_error_Delta','score_Delta',
                              'mlss_Delta','mlsl_Delta','mlus_Delta','mlul_Delta'))
