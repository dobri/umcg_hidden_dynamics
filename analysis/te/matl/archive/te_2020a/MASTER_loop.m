...
it both ways. The output is the TE values for Stimulus->Participant
and Participant->Stimulus and the corresponding surrogate test.
Here in each trial we will take the percentiles rel to the
bootstrapped distribution.

Group comparison? Not necessary and not easy although the toolbox
provides example scripts for that. The trick is to estimate common
parameters so that the TE values can be comparable across conditions. 
To begin with, the phase spaces will be of different size and dimension 
between conditions, affecting the meaning of a 'bit' of mutual information.
%}
...
switch str
    case 'PCWIN64'
        % your folder structure here for a Win PC.
        root_folder = 'C:\\User\...';
    case 'GLNXA64'
        root_folder = '~/c3/umcg';
end
InputDataPath = fullfile(root_folder,'data/training_processed/training');
OutputDataPath = fullfile(root_folder,'te','out');
cd(fullfile(root_folder,'te'))
addpath(fullfile(root_folder,'te'))
addpath(fullfile(root_folder,'te','libs','TRENTOOL3'))
addpath(fullfile(root_folder,'te','libs','fieldtrip'))
ft_defaults
...
for f=1:numel(DATA)
    figure(1)
    s=dlmread(fullfile(InputDataPath,DATA{f}.pp{1},'scores'),',',0,0);
    subplot(1,2,1)
    plot(TE_rez{f}.surr_test','-o')
    hold on
    plot(s,'-s','linewidth',2)
    legend('TE_{Stim->User}','TE_{User->Stim}','Performance score')
    hold off
    subplot(1,2,2)
    scatter(s,TE_rez{f}.surr_test(1,:)','b')
    hold on
    scatter(s,TE_rez{f}.surr_test(2,:)','r')
    hold off
