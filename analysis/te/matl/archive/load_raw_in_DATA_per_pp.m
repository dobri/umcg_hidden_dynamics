...
clear
pps = dir('PP*')';
A = table('Size',[3000,14],'VariableTypes',["string","int8","int8","string",'int8','string','logical','double','int32','double','double','double','double','int8']);
...
for p = pps
    ...
    for d = 1:2
        ...
        for s = 1:size(scores,1)
            ...
            DATA{ppn}.trial{counter_trial} = [x x]';
            DATA{ppn}.scores(counter_trial,:) = A{c,10:13};
            DATA{ppn}.pp{counter_trial} = A{c,2};
            DATA{ppn}.file_name{counter_trial} = A{c,1};
            DATA{ppn}.conditions(counter_trial,:) = A{c,[5 7 14]};
            DATA{ppn}.time{counter_trial} = t';
            DATA{ppn}.fsample(counter_trial) = 1/mean(diff(t));
            DATA{ppn}.label = {'Stim','User'};
            DATA{ppn}.tau(:,counter_trial) = [tau_stim; tau_pp];
            DATA{ppn}.dim(:,counter_trial) = [m_stim; m_pp];
        end
    end
end
A(c+1:end,:) = [];
