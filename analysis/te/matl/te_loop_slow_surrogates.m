...
for p=1:numel(isnotemptyvec)
    pp=isnotemptyvec(p);
    TEtable = vertcat(TEtable,[ones(size(DATA{pp}.conditions,1),1)*pp DATA{pp}.conditions,...
...
column_labels = {'PP','Date','Time','Task','Auditory','Visual','epsilon',...
    'trial',...
    'TEstim','TEpp',...
...
%writetable(TEtable,['te_trial_shuffle_surrs_allpps_' datestr(now,'yyyymmdd-HHMMSS') '.csv'],'Delimiter',',')
