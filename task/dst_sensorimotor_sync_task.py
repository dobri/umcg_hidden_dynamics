...
if __name__ == '__main__':
    ...
    try:
        ...
        print('\n')
        print('Some options are not typed correctly. Check with dst_sensorimotor_sync_task.py -h')
        print('\n')
        sys.exit(2)
    ...
    # Some bad interaction between cv2 windows and matplotlib in Python3 on my system.        
    if VIS_MODALITY:
        MOVT_PLOTTING=False
    if CAM_VIS_FEEDBACK:
        MOVT_PLOTTING=False
    ...
    if (comp[1]=='pop-os') | (comp[1]=='ideapad720s'):
        ...
        OFFSETS[:,:,0] = [[-5.,-5.,3.],[-106.,101.,100.]]
        # The red wii controller!
        OFFSETS[:,:,1] = [[-25.,-28.,-26.],[-122.,68.,70.]]
        #OFFSETS[:,:,0] = [[-25.,-30.,-25.],[-121.,68.,70.]]
    else:
        # The white wii controller!
        OFFSETS[:,:,1] = [[-5.,-5.,3.],[-106.,101.,100.]]
        # The red wii controller!
        OFFSETS[:,:,0] = [[-25.,-28.,-26.],[-122.,68.,70.]]
        #OFFSETS[:,:,0] = [[-25.,-30.,-25.],[-121.,68.,70.]]
    ...
    if SOUND_FLAG:
        ...
        MIDINOTER = np.ndarray.item( np.random.uniform(.01,1,1) ) * 20 + 40
        MIDINOTEL = np.ndarray.item( np.random.uniform(.01,1,1) ) * 20 + 40
